...
class MD5(object):
    ...
    def _step_5(cls):
        ...
        return bytes(f"{format(A, '08x')}{format(B, '08x')}{format(C, '08x')}{format(D, '08x')}", 'utf-8')


def hmac(message, key, block_size=64):
    ...
    i_pad = [0x36 * block_size]
    o_pad = [0x5c * block_size]

    i_key_pad = ''.join([chr(x ^ y) for x, y in zip(key, i_pad)])
    o_key_pad = ''.join([chr(x ^ y) for x, y in zip(key, o_pad)])
    
    i_key_pad_message = str(MD5.hash(i_key_pad + message))
    result = MD5.hash(o_key_pad + i_key_pad_message)
